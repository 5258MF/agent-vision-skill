...
`-g` 表示安装到用户级目录，`-y` 跳过交互确认，`--copy` 使用文件复制以避免 Windows 符号链接权限问题。安装完成后，重新打开智能体会话或刷新 Skill 索引。
...
下载仓库后，将整个 `skills/agent-vision` 目录复制到框架支持的 Skill 目录，并保持目录名为 `agent-vision`。常见位置：
